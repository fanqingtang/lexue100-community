  ...
  <header class="header">
    <div class="content">
      ...
      <nav class="tabGroup">
        ...
        <li>
          <a :href="http+'#/report'">小乐快报</a>
        </li>
        <li>
          <a :href="http+'#/fanWorld'">乐粉天地</a>
        </li>
        <!-- './student.php?do=index&randstr='+anyNumber -->
        <li>
          <a :href="anyNumber" target="_blank">学习专区</a>
        </li>
      </nav>
      <div class="iconGroup">
        <a :href="http+'#/publishArticle'" class="pen"></a>
        <a class="message" @click="noticeClick()" ref="noticeButton">
          <span v-show="noticeNum > 0">{{limitNoticeNum}}</span>
        </a>
        <a href="mk.php?do=xlsq#/studentCenter/" v-if="authCookie">
          <img :src="user.avator" class="avatar">
        </a>
            ...
            <div class="noticesList">
              <ul>
                <li v-for="(item , index) in noticeList" :key="index">
                  <a :href="http+'#/notices'" v-html="item"></a>
                </li>
              </ul>
            </div>
            <div class="moreWrap">
              <a :href="http+'#/notices'" class="more">查看所有通知</a>
            </div>
          </main>
        </div>
        <!--通知浮层 end-->
