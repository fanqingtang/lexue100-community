...
export function throttle(fn , delay) {
    ...
    return function() {
       ...
       //判断上次触发的时间和本次触发的时间差是否小于时间间隔的阔值
       if(now - last < delay) {
         //如果时间间隔小于我们设定的时间间隔阔值，则为本次触发操作设立一个新的定时器
         clearTimeout(timer);
         ...
         //如果时间间隔超出了我们设定的时间间隔阔值，那就不等了，无论如何要反馈给用户一次响应
         last = now;
         fn.apply(context , args);  
       }
    }
